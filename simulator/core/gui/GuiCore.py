...
class GuiCore:
    ...
    def text(self, surface, text, color, pos):
    ...
    def createButton(self, surface, text, state, pos, callback):
        ...
        if state:
            pygame.draw.rect(surface, (160,235,88), (pos[0]+border, pos[1]+border, w-2*border, h-2*border))
            self.text(surface, text, (0,0,0), (textX, textY))
        else:
            pygame.draw.rect(surface, (0, 62, 82), (pos[0]+border, pos[1]+border, w-2*border, h-2*border))
            self.text(surface, text, (255,255,255), (textX, textY))
